...
use integration_signal::{SignalClient, SignalError};
...
mod config_tests {
    ...
    fn config_new_sets_defaults() {
        ...
        assert_eq!(
            config.socket_path,
            SignalClientConfig::DEFAULT_SOCKET_PATH
        );
    ...
    fn config_with_data_path() {
        let config = SignalClientConfig::new("+1234567890")
            .with_data_path("/var/lib/signal-cli");
...
mod jsonrpc_tests {
    use integration_signal::{JsonRpcRequest, JsonRpcResponse, JsonRpcError};
...
mod send_params_tests {
    ...
    fn text_with_reply_params() {
        let params = SendParams::text("+1234567890", "Reply text")
            .with_reply(1234567890, "+0987654321");
        let json = serde_json::to_string(&params).unwrap();
...
mod receipt_params_tests {
    use integration_signal::{SendReceiptParams, ReceiptType};
